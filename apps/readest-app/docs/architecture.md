...
flowchart TB
    ...
    Send["app/send<br/>(local/extension capture)"]
    User["app/user<br/>(account and settings)"]
    Updater["app/updater"]
    Offline["app/offline"]
...
The former paid email-ingest worker has been removed. The `Send` page and
browser-extension capture endpoints remain for user-initiated imports.
...
1. The browser extension or the local Send page submits a URL or article HTML.
