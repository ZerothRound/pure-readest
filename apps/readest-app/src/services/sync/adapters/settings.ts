...
export const SETTINGS_WHITELIST = [
  ...
  // WebDAV connection. serverUrl + paths + format sync as plaintext so a
  // fresh device pre-fills the connect form and uses the same Legado tree;
  // username / password are listed in `encryptedFields` below. Per-device
  // bookkeeping (enabled, deviceId, lastSyncedAt, sync sub-toggles) is
  // deliberately excluded — see KOSync, which likewise syncs credentials
  // but not its `enabled` flag.
  'webdav.serverUrl',
  'webdav.username',
  'webdav.password',
  'webdav.rootPath',
  'webdav.syncFormat',
  'webdav.legadoRootPath',
