...
// Auth state remains in the harness for unrelated reader controls; offline
// audio itself is deliberately independent of it.
...
describe('TTSPlayerSheet', () => {
  ...
  test('an aligned engine swaps the cover layout for the lyric sheet', async () => {
    getBookData.mockReturnValue({
      book: { title: 'Alice in Wonderland', coverImageUrl: 'blob:cover' },
    });
  ...
  test('offline audio row has no paid gate and opens the chapters view', () => {
    mockQuota.userProfilePlan = 'pro';
  ...
  test('offline audio row is available to a free user', () => {
  ...
  test('offline audio row is available while signed out', () => {
    mockAuth.user = null;
